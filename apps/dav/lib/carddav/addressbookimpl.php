...
class AddressBookImpl implements IAddressBook {
	...
	public function search($pattern, $searchProperties, $options) {
		$result = $this->database->searchContact($this->getKey(), $pattern, $searchProperties);

		$vCards = [];
		foreach ($result as $cardData) {
			$vCards[] = $this->vCard2Array(Reader::read($cardData));
		}
	...
	public function delete($id) {
		$uri = $this->database->getCardUri($id);
	...
	protected function createUid() {
		...
		} while (!empty($this->database->getContact($uid . '.vcf')));
